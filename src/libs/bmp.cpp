...
static void __CV_STD_BMP_LOAD(
   ...
   if( !CV::ErrorCheck::ExpectNoPrefixer(name, args, token, cursor) ||
    ...
    auto handle = CV::Build::Store();
    auto pixels = CV::Build::List();

    handle->v["width"] = CV::Build::Number(width);
    handle->v["height"] = CV::Build::Number(height);
    handle->v["channels"] = CV::Build::Number(nrChannels);
    handle->v["data"] = pixels;

    int total = width * height * nrChannels;
    for(int i = 0; i < total; ++i){
        pixels->v.push_back(CV::Build::Number(data[i]));
    }
