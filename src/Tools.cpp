...
bool CV::Tools::importBinaryLibrary(const std::string &path, const std::string &fname, const std::shared_ptr<CV::Stack> &stack){
    #if (_CV_PLATFORM == _CV_PLATFORM_TYPE_LINUX)
        ...
        return true;
    #elif  (_CV_PLATFORM == _CV_PLATFORM_TYPE_WINDOWS)
        typedef void (*rlib)(const std::shared_ptr<CV::Stack> &stack);
